...
function wordnotfound( $source_word, $source_code, $source_language_name, $target_language_name ) {
	...
	$this->show_header('404 - Word Not Found');
	if( $_SESSION['level'] >= 1 ) { 
		print ''
		. '<form method="POST" action="' . HOSTNAME . 'task/">'
		. '<p>Sugggest a new translation:</p>'
		. '<p>Language pair: ' . $this->get_language_name( $source_code ) . ' = '
		. $target_language_name . '</p>'
		. '<p>Word pair: <b>' . $source_word . '</b> = '
		. '<input type="hidden" name="link" value="add">'
		. '<input type="hidden" name="source_language_code" value="' . $source_code .'" />'
		. '<input type="hidden" name="source_word" value="' . urlencode($source_word) . '" />'
		. '<input type="hidden" name="target_language_code" value="' 
		. $this->get_language_code( $target_language_name ) . '" />'
		. '<input type="input" name="target_word" size="20" maxlength="127" value="">'
